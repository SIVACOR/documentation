...
- `jetstream2-nodes.csv` — worker sizes. `disk_fs_gib` is what `df` reports on a worker;
  ...
  `upload_max_gb` / `user_quota_gb` are the submission site's per-archive and per-user limits;
  `relative_cost` is the charge relative to the default size, `by_request` marks sizes that must be requested.
